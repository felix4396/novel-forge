...
export function buildPlannerActiveSkillsBlock(activeSkills: RuntimeActiveSkill[]): string {
  if (activeSkills.length === 0) {
    return "无";
  }
  return [
    "启用 Skills（Planner 必须将类型规则、状态要求和章节规划钩子纳入计划；若与硬状态冲突，以硬状态和人工确认优先）：",
    ...activeSkills.slice(0, 8).flatMap((skill) => {
      const lines = [
        `- ${skill.name} (${skill.category}, priority=${skill.priority}, conflict=${skill.conflictStatus})`,
        skill.promptHooks.planBook ? `  开书规划钩子：${skill.promptHooks.planBook}` : "",
        skill.promptHooks.planChapter ? `  章节规划钩子：${skill.promptHooks.planChapter}` : "",
        ...
        skill.riskTriggers.length > 0 ? `  风险触发：${skill.riskTriggers.slice(0, 8).join(" / ")}` : "",
        skill.conflictStatus !== "ok" && skill.conflictStatus !== "disabled"
          ? `  冲突证据：${skill.conflictJson}`
          : "",
      ].filter(Boolean);
      return lines;
    }),
...
export function buildBookPlanContextBlocks(input: {
  ...
  return [
    ...
    createContextBlock({
      ...
      group: "active_skills",
      priority: 94,
      content: buildPlannerActiveSkillsBlock(input.activeSkills ?? []),
    }),
    createContextBlock({
      id: "chapter_drafts",
