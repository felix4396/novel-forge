...
export class PlannerService {
  ...
  async generateBookPlan(novelId: string, options: PlannerOptions = {}) {
    ...
    const [styleEngine, activeSkills] = await Promise.all([
      this.resolvePlannerStyleEngineSummary(novelId, undefined, options.taskStyleProfileId),
  ...
  async generateChapterPlan(novelId: string, chapterId: string, options: GenerateChapterPlanOptions = {}) {
    const [novel, chapter, bible, plotBeats, summaries, characters, bookPlan, arcPlans, volumePlans, recentAuditReports, recentDecisions, storyMacroPlanRow, styleEngine, pendingReviewProposalCount, activeSkills] = await Promise.all([
