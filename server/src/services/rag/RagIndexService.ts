import type { KnowledgeIndexStatus, RagIndexJob } from "@prisma/client";
...
export class RagIndexService {
    ...
    switch (ownerType) {
      ...
      case "reference_corpus": {
        const corpus = await prisma.referenceCorpus.findUnique({
          ...
          include: {
            chunks: {
              where: { chunkType: { in: ["paragraph", "chapter", "summary", "style_candidate"] } },
        ...
        const preChunks = corpus.chunks.map((chunk) => ({
          chunkText: normalizeRagText(chunk.text),
          facets: {
            ...(chunk.chapterIndex !== null ? { chapterAnchor: [String(chunk.chapterIndex)] } : {}),
          },
          anchor: {
            chapterIndex: chunk.chapterIndex ?? undefined,
            excerptOffsetRange: {
          ...
          metadata: {
            ...
            ...parseRecordJson(chunk.metadataJson),
          },
        })).filter((item) => item.chunkText.length > 0);
        const content = normalizeRagText(corpus.content);
        return content
          ? [{
            ownerType,
            ownerId,
  ...
  private async collectOwners(scope: ReindexScope, id?: string): Promise<PendingOwner[]> {
    ...
    if (scope === "novel" || scope === "all") {
      ...
      const [chapters, summaries, facts, characters, timelines, referenceCorpora] = await Promise.all([
        ...
        prisma.referenceCorpus.findMany({
          where: { novelId: { in: novelIds }, status: "active" },
          select: { id: true },
        }),
      ]);
      ...
      referenceCorpora.forEach((item) => push("reference_corpus", item.id));
    }
  ...
  async updateJobStatus(jobId: string, payload: {
    ...
    await this.syncOwnerIndexStatus(
  ...
  private async syncOwnerIndexStatus(
    ownerType: RagOwnerType,
    ownerId: string,
    status: RagJobStatus,
    jobType: RagJobType,
  ): Promise<void> {
    const nextStatus: KnowledgeIndexStatus = jobType === "delete" && (status === "succeeded" || status === "cancelled")
      ? "idle"
      : status === "queued"
        ? "queued"
        : status === "running"
    ...
    const data: { latestIndexStatus: KnowledgeIndexStatus; lastIndexedAt?: Date } = {
    ...
    if (ownerType === "reference_corpus") {
      await prisma.referenceCorpus.updateMany({
        where: { id: ownerId },
        data,
      });
    }
