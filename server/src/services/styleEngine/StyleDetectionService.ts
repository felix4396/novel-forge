...
export class StyleDetectionService {
  ...
  async check(input: DetectionInput): Promise<StyleDetectionReport> {
    ...
    const enabledAntiRules = antiRules.filter((rule) => rule.enabled);
    const forbiddenRules = enabledAntiRules.filter((rule) => rule.type === "forbidden");
    const literalPatterns = forbiddenRules.flatMap((rule) => (
      rule.detectPatterns.filter((pattern) => !/[\\^$.*+?()[\]{}|]/.test(pattern))
    ));
    const literalRuleCount = forbiddenRules.filter((rule) => (
      rule.detectPatterns.some((pattern) => !/[\\^$.*+?()[\]{}|]/.test(pattern))
    )).length;
    const canUseLiteralOnlyFastPath = !styleContractText
      && enabledAntiRules.length === forbiddenRules.length
      && forbiddenRules.length > 0
      && literalRuleCount === forbiddenRules.length;
    if (canUseLiteralOnlyFastPath && literalPatterns.length > 0) {
