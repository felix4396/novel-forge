...
export class AntiAiPolicyResolver {
  ...
  private async listContextBindings(input: { novelId: string; volumeId?: string; chapterId?: string }): Promise<StyleBinding[]> {
    const resolvedVolumeId = input.volumeId?.trim()
      || await this.findChapterVolumeId(input.chapterId?.trim());
    const rows = await prisma.styleBinding.findMany({
      where: {
        enabled: true,
        OR: [
          { targetType: "novel", targetId: input.novelId },
          ...(resolvedVolumeId ? [{ targetType: "volume" as const, targetId: resolvedVolumeId }] : []),
  ...
  private async findChapterVolumeId(chapterId?: string): Promise<string | null> {
    if (!chapterId) {
      return null;
    }
    const volumeChapter = await prisma.volumeChapterPlan.findFirst({
      where: { chapterId },
      select: { volumeId: true },
      orderBy: [{ updatedAt: "desc" }, { id: "desc" }],
    });
    return volumeChapter?.volumeId ?? null;
  }
