...
import { buildPlannerActiveSkillsBlock } from "../../planner/plannerContextBlocks";
...
export class ChapterPlanJITService {
  ...
  async ensureExecutionReady(novelId: string, chapterId: string): Promise<void> {
    ...
    // 拉取前文事实账本与当前启用 Skills，供 JIT Planner 生成 task sheet。
    const [facts, activeSkills] = await Promise.all([
      novelFactService.listForChapter({
        novelId,
        beforeChapterOrder: chapter.order,
      }),
      skillRuntimeContextService.getActiveSkills(novelId).catch(() => []),
    ]);
    ...
    if (hasCompleteTaskSheet && facts.length >= JIT_MIN_FACTS_FOR_REFRESH) {
      ...
      const guidance = buildJitPlannerGuidance({
        facts,
        activeSkills,
      });
...
function buildJitPlannerGuidance(input: {
  facts: Awaited<ReturnType<typeof novelFactService.listForChapter>>;
  activeSkills: Awaited<ReturnType<typeof skillRuntimeContextService.getActiveSkills>>;
}): string {
  return [
    input.facts.length > 0 ? buildFactLedgerGuidance(input.facts) : "",
    input.activeSkills.length > 0
      ? [
          "【启用 Skills — 请将以下类型规则、状态要求和规划钩子写入 task sheet 设计】",
          buildPlannerActiveSkillsBlock(input.activeSkills),
        ].join("\n")
      : "",
