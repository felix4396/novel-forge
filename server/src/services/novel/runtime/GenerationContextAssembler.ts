...
function buildRagContextBlockFromRows(rows: RetrievedChunk[]): string {
  if (rows.length === 0) {
    return "";
  }
  return rows
...
function buildRuntimeRagReferences(rows: RetrievedChunk[]): GenerationContextPackage["ragReferences"] {
  return rows.map((item) => ({
    ...
    snippet: compactSnippet(item.chunkText, 260),
    contextPrefix: item.contextPrefix ?? null,
  }));
}
...
export class GenerationContextAssembler {
  ...
  async assemble(
    ...
    let ragText = "";
    let ragReferences: GenerationContextPackage["ragReferences"] = [];
    try {
      const ragRows = await ragServices.hybridRetrievalService.retrieve(ragQuery, {
        novelId,
        currentChapterOrder: chapter.order,
      });
      ragText = buildRagContextBlockFromRows(ragRows);
      ragReferences = buildRuntimeRagReferences(ragRows);
    } catch {
      ragText = "";
      ragReferences = [];
    }
