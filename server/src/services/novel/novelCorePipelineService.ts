...
import { aiWorkbenchAgentRunLogger } from "../aiWorkbench/AiWorkbenchAgentRunLogger";
import {
  logPipelineError,
  logPipelineInfo,
  logPipelineWarn,
...
export class NovelCorePipelineService {
  ...
  private async executePipeline(jobId: string, novelId: string, options: PipelineRunOptions) {
    ...
    const runtimePayload: PipelinePayload = {
      ...
      aiWorkbenchAgentRunId: persistedPayload.aiWorkbenchAgentRunId,
    ...
    let aiWorkbenchAgentRunId: string | null = shouldRecordDirectorTelemetry
      ? runtimePayload.aiWorkbenchAgentRunId ?? null
      : null;
    const createAiWorkbenchAgentRun = async () => aiWorkbenchAgentRunLogger.startRun({
      novelId,
      sessionId: `ai-workbench:auto-director:${runtimePayload.workflowTaskId ?? jobId}`,
      goal: `自动导演章节执行第 ${options.startOrder}-${options.endOrder} 章`,
      metadata: {
        source: "auto_director_pipeline",
        workflowTaskId: runtimePayload.workflowTaskId ?? null,
        generationJobId: jobId,
        startOrder: options.startOrder,
        endOrder: options.endOrder,
      },
    });
    const persistAiWorkbenchAgentRunId = async () => {
      if (!aiWorkbenchAgentRunId) return;
      runtimePayload.aiWorkbenchAgentRunId = aiWorkbenchAgentRunId;
      await this.updateJobSafe(jobId, {
        payload: this.stringifyPipelinePayload(runtimePayload),
      });
    };
    if (shouldRecordDirectorTelemetry && !aiWorkbenchAgentRunId) {
      aiWorkbenchAgentRunId = await createAiWorkbenchAgentRun();
      await persistAiWorkbenchAgentRunId();
    } else if (aiWorkbenchAgentRunId) {
      try {
        await aiWorkbenchAgentRunLogger.markRunning({
          runId: aiWorkbenchAgentRunId,
          currentStep: "Planner",
    ...
    let requiresWorkbenchApproval = false;
      ...
      await runWithLlmUsageTracking({
        ...
        for (let chapterIndex = 0; chapterIndex < chaptersToProcess.length; chapterIndex++) {
          ...
          if (aiWorkbenchAgentRunId) {
            await aiWorkbenchAgentRunLogger.addStep({
              ...
              output: {
                chapterOrder: chapter.order,
                taskStyleProfileId: runtimePayload.taskStyleProfileId ?? null,
                controlAdvanceMode: runtimePayload.controlPolicy?.advanceMode ?? null,
              },
              provider: runtimePayload.provider,
              model: runtimePayload.model,
            }).catch(() => undefined);
            await aiWorkbenchAgentRunLogger.addStep({
              ...
              output: {
                contentLength: chapterResult.runtimePackage?.draft.content.length ?? null,
                retryCountUsed: chapterResult.retryCountUsed,
              },
              provider: runtimePayload.provider,
              model: runtimePayload.model,
            }).catch(() => undefined);
          ...
          if (chapterResult.reviewExecuted) {
            await createQualityReport(novelId, chapter.id, final.score, final.issues, {
              sourceType: chapterResult.retryCountUsed > 0 ? "repair_recheck" : "pipeline_review",
              contentLength: (chapterResult.runtimePackage?.draft.content ?? chapter.content ?? "").length,
              agentRunId: aiWorkbenchAgentRunId,
            });
            if (aiWorkbenchAgentRunId) {
              await aiWorkbenchAgentRunLogger.addStep({
                runId: aiWorkbenchAgentRunId,
                ...
                output: {
                  reviewExecuted: chapterResult.reviewExecuted,
          ...
          if (chapterResult.reviewExecuted && !chapterResult.pass) {
            requiresWorkbenchApproval = true;
        ...
        if (aiWorkbenchAgentRunId) {
          await aiWorkbenchAgentRunLogger.finishRun({
            runId: aiWorkbenchAgentRunId,
            status: requiresWorkbenchApproval ? "waiting_approval" : "succeeded",
          }).catch(() => undefined);
        }
      });
    } catch (error) {
