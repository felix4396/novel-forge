...
function buildActiveSkillsContextBlock(writeContext: ChapterWriteContext): string {
  return [
    "启用的写作 Skills（本章必须按优先级执行；如与章节任务冲突，以章节任务和硬状态为准）：",
    ...writeContext.activeSkills.slice(0, 8).map((skill) => {
      const hooks = [
        skill.promptHooks.planChapter ? `规划钩子=${skill.promptHooks.planChapter}` : "",
        skill.promptHooks.write ? `写作钩子=${skill.promptHooks.write}` : "",
        skill.promptHooks.review ? `验收钩子=${skill.promptHooks.review}` : "",
      ].filter(Boolean);
      return [
        `- ${skill.name} [${skill.slug}; priority=${skill.priority}; conflict=${skill.conflictStatus}]`,
        skill.description ? `  说明：${compactText(skill.description)}` : "",
        skill.stateRequirements.length > 0 ? `  状态要求：${skill.stateRequirements.slice(0, 8).join(" / ")}` : "",
        hooks.length > 0 ? `  Prompt Hooks：${hooks.join(" | ")}` : "",
        skill.reviewGateChecks.length > 0 ? `  ReviewGate 检查：${skill.reviewGateChecks.slice(0, 8).join(" / ")}` : "",
        skill.riskTriggers.length > 0 ? `  触发人工确认风险：${skill.riskTriggers.slice(0, 8).join(" / ")}` : "",
        skill.conflictStatus !== "ok" && skill.conflictJson !== "[]"
          ? `  Skill 冲突证据：${skill.conflictJson}`
          : "",
      ].filter(Boolean).join("\n");
    }),
  ].join("\n");
...
export function buildChapterWriterContextBlocks(
  ...
  const includeActiveSkills = writeContext.activeSkills.length > 0;
  ...
  const blocks: Array<PromptContextBlock | null> = [
    ...
    includeActiveSkills
      ? createContextBlock({
        ...
        group: "active_skills",
        priority: 99,
        required: mode === "full" || mode === "review" || mode === "repair",
        allowSummary: false,
        content: buildActiveSkillsContextBlock(writeContext),
      })
      : null,
