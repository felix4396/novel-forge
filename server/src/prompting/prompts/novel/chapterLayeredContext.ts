...
export function buildChapterWriteContext(input: {
  ...
  return {
    ...
    activeSkills: input.contextPackage.activeSkills ?? [],
    ragFacts: [],
    completedMilestones: [],
    recentScenePatterns: [],
  };
...
export function buildChapterReviewContext(
  ...
  return {
    ...
    structureObligations: takeUnique([
      ...
      ...writeContext.activeSkills.flatMap((skill) => (
        skill.reviewGateChecks.map((check) => `skill review gate (${skill.name}): ${check}`)
      )),
      ...writeContext.activeSkills.flatMap((skill) => (
        skill.riskTriggers.map((trigger) => `skill risk trigger (${skill.name}): ${trigger}`)
      )),
...
export function buildChapterRepairContext(input: {
  ...
  return {
    ...
    structureObligations: takeUnique([
      ...
      ...writeContext.activeSkills.flatMap((skill) => (
        skill.reviewGateChecks.map((check) => `skill repair gate (${skill.name}): ${check}`)
      )),
      ...writeContext.activeSkills.flatMap((skill) => (
        skill.riskTriggers.map((trigger) => `skill repair risk trigger (${skill.name}): ${trigger}`)
      )),
      ...writeContext.ledgerPendingItems.map((item) => buildLedgerItemLine(item, "pending payoff")),
      ...writeContext.ledgerUrgentItems.map((item) => buildLedgerItemLine(item, "urgent payoff")),
      ...writeContext.ledgerOverdueItems.map((item) => buildLedgerItemLine(item, "overdue payoff")),
    ...
    allowedEditBoundaries: takeUnique([
      ...
      writeContext.pendingCandidateGuards.length > 0
        ...
        : "",
      ...writeContext.activeSkills.flatMap((skill) => (
        skill.promptHooks.write ? [`Keep active skill writing hook (${skill.name}): ${skill.promptHooks.write}`] : []
      )),
      ...writeContext.activeSkills.flatMap((skill) => (
        skill.reviewGateChecks.map((check) => `Preserve active skill ReviewGate (${skill.name}): ${check}`)
      )),
