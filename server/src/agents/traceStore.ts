...
function parseStepTokenUsage(raw: string | null): {
  promptTokens: number;
  ...
  try {
    ...
    const usage = extractLlmTokenUsage({ usageMetadata: parsed })
      ?? extractLlmTokenUsage(parsed);
    if (!usage) {
      return null;
    }
    return usage;
...
export class AgentTraceStore {
  private async recordModelCallLogForStep(row: {
    id: string;
    runId: string;
    seq: number;
    agentName: string;
    ...
    if (!row.provider?.trim() || !row.model?.trim() || !row.tokenUsageJson?.trim()) {
      return;
    }
    const usage = parseStepTokenUsage(row.tokenUsageJson);
    if (!usage) {
      return;
    }
    await prisma.modelCallLog.create({
      data: {
        novelId: row.run?.novelId ?? null,
        agentRunId: row.runId,
        agentStepId: row.id,
        taskType: `agent.${row.stepType}`,
        provider: row.provider,
        model: row.model,
        status: row.status,
  ...
  async addStep(input: {
    ...
    await this.recordModelCallLogForStep(row).catch(() => undefined);
