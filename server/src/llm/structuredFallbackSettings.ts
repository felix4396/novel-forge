...
const STRUCTURED_FALLBACK_MAX_REPAIR_ATTEMPTS_KEY = "structuredFallback.maxRepairAttempts";
...
function normalizeMaxRepairAttempts(value: number | string | undefined | null): number {
  const numeric = typeof value === "number" ? value : Number(value);
  if (!Number.isFinite(numeric)) {
    return DEFAULT_STRUCTURED_FALLBACK_SETTINGS.maxRepairAttempts;
  }
  return Math.min(5, Math.max(0, Math.floor(numeric)));
}
...
function buildSettingsFromEntries(entries: Map<string, string>): StructuredFallbackSettings {
  return {
    ...
    maxRepairAttempts: normalizeMaxRepairAttempts(entries.get(STRUCTURED_FALLBACK_MAX_REPAIR_ATTEMPTS_KEY)),
  };
}
      ...
      where: {
        key: {
          in: [
            ...
            STRUCTURED_FALLBACK_MAX_REPAIR_ATTEMPTS_KEY,
          ],
        },
      },
    });
...
export async function saveStructuredFallbackSettings(input: Partial<StructuredFallbackSettings>): Promise<StructuredFallbackSettings> {
  ...
  const next: StructuredFallbackSettings = {
    ...
    maxRepairAttempts: normalizeMaxRepairAttempts(input.maxRepairAttempts ?? previous.maxRepairAttempts),
  };
  try {
    await prisma.$transaction([
      ...
      prisma.appSetting.upsert({
        where: { key: STRUCTURED_FALLBACK_MAX_REPAIR_ATTEMPTS_KEY },
        update: { value: String(next.maxRepairAttempts) },
        create: { key: STRUCTURED_FALLBACK_MAX_REPAIR_ATTEMPTS_KEY, value: String(next.maxRepairAttempts) },
      }),
    ]);
    cachedSettings = next;
    return next;
