...
export async function invokeStructuredLlmDetailed<T>(input: StructuredInvokeInput<T>): Promise<StructuredInvokeResult<T>> {
  ...
  const effectiveInput: StructuredInvokeInput<T> = {
    ...input,
    maxRepairAttempts: input.maxRepairAttempts ?? fallbackSettings?.maxRepairAttempts ?? 1,
  };
  const primaryTarget = await resolveAttemptTarget({
    provider: effectiveInput.provider,
    model: effectiveInput.model,
    apiKey: effectiveInput.apiKey,
