...
async function recordModelCallLog(input: {
  context: LlmUsageTrackingContext;
  usage: LlmTokenUsageSnapshot;
  record?: TrackedUsageRecordInput;
}): Promise<void> {
  const provider = input.record?.meta?.provider ? String(input.record.meta.provider).trim() : "";
  const model = input.record?.meta?.model?.trim() ?? "";
  if (!provider || !model) {
    return;
  }
  const promptMeta = input.record?.meta?.promptMeta;
  const taskType = input.record?.meta?.taskType
    ?? promptMeta?.promptId
    ?? input.context.directorNodeKey
    ?? "llm.tracked";
  await prisma.modelCallLog.create({
    data: {
      ...
      provider,
      model,
      status: input.record?.status ?? "succeeded",
      promptTokens: input.usage.promptTokens,
      completionTokens: input.usage.completionTokens,
      totalTokens: input.usage.totalTokens,
      latencyMs: typeof input.record?.durationMs === "number"
        ? Math.max(0, Math.round(input.record.durationMs))
        : null,
      metadataJson: JSON.stringify({
        source: "llm_usage_tracking",
        workflowTaskId: input.context.workflowTaskId ?? null,
        generationJobId: input.context.generationJobId ?? null,
        styleExtractionTaskId: input.context.styleExtractionTaskId ?? null,
        directorRunId: input.context.directorRunId ?? null,
        directorNodeKey: input.context.directorNodeKey ?? null,
        modelRoute: input.record?.meta?.modelRoute ?? null,
        routeDegraded: input.record?.meta?.routeDegraded === true,
