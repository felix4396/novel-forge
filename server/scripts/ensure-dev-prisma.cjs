...
function canInstantiatePrismaClient(runtimeConfig) {
  const escapedRuntimeConfig = JSON.stringify(runtimeConfig);
  const result = runNodeProbe(
    `
    const runtimeConfig = ${escapedRuntimeConfig};
    const { PrismaClient } = require("@prisma/client");
    let adapter;
    if (runtimeConfig.provider === "sqlite") {
      const { PrismaBetterSqlite3 } = require("@prisma/adapter-better-sqlite3");
      adapter = new PrismaBetterSqlite3({ url: runtimeConfig.url });
    } else {
      const { PrismaPg } = require("@prisma/adapter-pg");
      adapter = new PrismaPg({ connectionString: runtimeConfig.url });
    }
...
function main() {
  ...
  const missingGeneratedClient = !fs.existsSync(generatedClientPath)
    || !canLoadPrismaClient()
    || !canInstantiatePrismaClient(runtimeConfig);
