...
export function useAutoDirectorCreateController(input: UseAutoDirectorCreateControllerInput) {
  ...
  const setLLMSelection = useLLMStore((state) => state.setSelection);
  ...
  useEffect(() => {
    ...
    const seedPayloadLlm = seedPayload as (typeof seedPayload & {
      provider?: unknown;
      model?: unknown;
    }) | null | undefined;
    ...
    if (typeof seedPayloadLlm?.provider === "string" || typeof seedPayloadLlm?.model === "string") {
      const currentLlm = llmRef.current;
      setLLMSelection({
        provider: (typeof seedPayloadLlm.provider === "string" && seedPayloadLlm.provider.trim()
          ? seedPayloadLlm.provider.trim()
          : currentLlm.provider) as LLMProvider,
        model: typeof seedPayloadLlm.model === "string" ? seedPayloadLlm.model.trim() : currentLlm.model,
        temperature: currentLlm.temperature,
        maxTokens: currentLlm.maxTokens,
      });
    }
  ...
  }, [applyAutoApprovalSnapshot, restoredTask, setLLMSelection, workflowTaskId]);
